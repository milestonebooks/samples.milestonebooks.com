    ...
    private function GetSeries($data): object {
        ...
        if ($res && $res->num_rows) {
            ...
            if ($data->isGroup) {
                ...
                $sql = "SELECT ig.group_item_id, gi.contains
                        FROM a01i_ItemsGroup ig
                        LEFT JOIN a01i_Items i ON (i.id = ig.group_item_id)
                        LEFT JOIN (
                            SELECT group_item_id, COUNT(*) contains
                            FROM a01i_ItemsGroup
                            GROUP BY group_item_id
                        ) gi ON (gi.group_item_id = ig.group_item_id)
                        WHERE ig.item_id = $item_id
                        AND i.is_active
                        ORDER BY gi.contains
                        LIMIT 1";
