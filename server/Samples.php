...
define('ROMAN_REGEX', "'^[ivxlcdm]+$'");

/*************************************************************************************************/
class Samples extends _Object2 {
    
    var $version = '2019-09-24';
    var $limit   = false;
    ...
    public function __construct($autorun=false) {
        ...
        if (!$_REQUEST['action']) $_REQUEST['action'] = '__html';
        
        parent::__construct($autorun);
    } // __construct()
    ...
    public function Data($item_id = null) {
        ...
        if (!$item_id || !is_numeric($item_id)) {
            ...
            $this->limit = $this->get('limit', $_REQUEST['limit'] && $_REQUEST['limit'] == 'true');
    ...
    private function GetSeries($data) {
        ...
        if ($res->num_rows) {
            ...
            if ($data->isGroup) {
                ...
                $group_item_id = (int)$this->QueryFetch($sql)->group_item_id;
            }
    ...
    private function GetSamples($data) {
    ...
    private function ParseFilename($file, $type, &$data) {
        ...
        //                  [1]id        [2]dpi     [3]format
        if (!preg_match("'\.([\w-]+)(?:\((\d+)\))?\.({$ext})$'", $file, $m)) return;
    ...
    private function IsSequential($sample, $samples) {
